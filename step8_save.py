...
This mirrors step8 (display) but saves the comparison grid to disk instead
of showing it. For each selected test sample, it renders:
  - Original image
  - Ground truth mask
...
Usage:
  python step8_save.py \
    ...
    [--num-samples 8] [--threshold 0.5] [--seed 42] [--mark-index 5] \
    [--out-dir artifacts/graphs] [--filename step8_comparison.png] [--per-sample]

Notes:
- Creates the output directory if it does not exist.
- When --per-sample is set, additionally saves one PNG per row.
"""
