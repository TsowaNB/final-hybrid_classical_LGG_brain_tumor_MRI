...
Evaluate the trained Classical U-Net (from step4b) on validation and test splits.
Loads the saved full H5 model by default, supports weights-only via JSON, sweeps
thresholds on the validation set to find the best binary mask threshold, and
reports metrics for validation and test.

Usage:
  python step5b.py --dataset-path <path> \
                   --model-path models/classical_unet_best.h5 \
                   [--arch-json models/classical_unet_arch.json]

Outputs:
  - artifacts/eval_metrics_classical.json: Summary of val/test metrics and best threshold
  - artifacts/threshold_sweep_classical.csv: Dice vs threshold on validation set
"""
